...
class PDFTableObject: PDFRenderObject {
        ...
        for (rowIdx, row) in mergeNodes.enumerated() {
            ...
            for node in row {
                ...
                var cell = calculate(generator: generator,
                                     ...
                                     origin: origin,
                                     width: width)
                cell = applyFixedRowHeightIfNeeded(to: cell, node: node)
    ...
    /**
     Overrides the calculated cell height when every spanned row has a fixed height.

     Rows without an entry in ``PDFTable/fixedRowHeights`` keep the content-based height.
     */
    func applyFixedRowHeightIfNeeded(to cell: PDFTableCalculatedCell, node: PDFTableNode) -> PDFTableCalculatedCell {
        let startRow = node.position.row
        let endRow = node.position.row + node.moreRowsSpan
        let spannedRows = startRow...endRow
        guard spannedRows.allSatisfy({ table.fixedRowHeights[$0] != nil }) else {
            return cell
        }

        var result = cell
        let heights = spannedRows.map { table.fixedRowHeights[$0]! }
        let fixedHeight = heights.reduce(0, +) + CGFloat(node.moreRowsSpan) * table.margin
        result.frames.cell.size.height = fixedHeight
        result.frames.content.size.height = min(
            result.frames.content.height,
            max(0, fixedHeight - 2 * table.padding)
        )
        return result
    }
